...
/** Extract the object UUID from a IIIF canvas ID of the form …/manifest/{uuid}/canvas/N */
function objectIdFromCanvasId(canvasId: string): string | null {
  const m = canvasId.match(/\/manifest\/([0-9a-f-]{36})\//i);
  return m ? m[1] : null;
}
...
function NotesPanel({
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-6">
      {/* Delete confirmation modal for canvas annotations */}
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
            <h3 className="text-base font-semibold text-slate-800 mb-2">Delete note?</h3>
            {note.canvas_id ? (
              <p className="text-sm text-slate-600 mb-5">
                This note is linked to a IIIF canvas annotation. Deleting it will permanently
                remove both the research note <strong>and the annotation marker</strong> from
                the image viewer. This cannot be undone.
              </p>
            ) : (
              <p className="text-sm text-slate-600 mb-5">
                This note and all its replies will be permanently deleted. This cannot be undone.
              </p>
            )}
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                onClick={onDelete}
                disabled={saving}
                className="text-sm px-4 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white font-medium transition-colors disabled:opacity-50"
              >
      ...
      <div className="flex items-start justify-between gap-4 mb-2">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            type="button"
            onClick={() => setConfirmDelete(true)}
            className="inline-flex items-center gap-1.5 text-sm text-red-400 hover:text-red-600 border border-red-100 hover:border-red-200 px-3 py-1.5 rounded-lg transition-colors"
            title="Delete note"
          >
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {/* IIIF viewer link for canvas annotations — use the linked object_id */}
      {note.canvas_id && note.object_ids.length > 0 && (
        <div className="mb-4">
          <a
            href={`/${locale}/objects/${note.object_ids[0]}?tab=viewer`}
            className="inline-flex items-center gap-1.5 text-xs font-medium text-indigo-600 hover:text-indigo-700 border border-indigo-200 hover:border-indigo-300 bg-indigo-50 hover:bg-indigo-100 px-3 py-1.5 rounded-lg transition-colors"
          >
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {note.canvas_id && (
          <span className="text-[10px] font-medium text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded-full">IIIF</span>
        )}
      </div>
    </button>
  );

...
export async function generateLabelPDF(
  ...
  const institutionName = options.institutionName ?? "Taisce Collection";
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    doc.text(`Generated by Taisce · Page ${p} of ${totalPages}`, PAGE.w / 2, PAGE.h - 2, { align: "center" });
  }
...
export async function generatePreviewDataUrl(
  ...
  await drawLabel(doc, bwipjs, obj, 2, 2, options.size, options.type, "Taisce Collection");
